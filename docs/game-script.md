...
Level 1 lines are copied from the shipped game (`dist/game.js`). Level 2 lines are **proposed** and not implemented; they support `level-2-plan.md`.
...
- Should the supervisor have a visible body, or only a voice and a red eye on the machinery?
- Does VELA return in Level 2 as the same dispatcher, or is she cut off by the supervisor's takeover?
- Pack's mode change (backpack vs. floating) affects animation lines. See `level-2-plan.md`.
