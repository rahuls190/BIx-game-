...
Status: specification, plus greybox placeholders for all 13 plates. No game code has changed.
...
| `bix-hang-v1.png` | 2172 × 724 | Already shipped but never referenced. Wire it up for hang and climb. |
...
| File | Size | Cells | Pri |
|---|---|---|---|
| `pack-assist-v1.png` | 1536 × 1024 | 4 × 2: hover idle · arm extended · scan (amber eye) · carry cell · ping burst · hold shutter · catch (arms out) · detach from back | P1 |
| `supervisor-head-v1.png` | 1536 × 1024 | 4 × 2: idle · sweep left · sweep right · locked (red) · stalled · gantry mount · emitter close · spare | P2 |
...
| File | Size | Cells | Pri |
|---|---|---|---|
| `enemy-crawler-v1.png` | 1536 × 1024 | 4 × 2: walk A–D · turn · alert · stunned · grab | P1 |
| `enemy-spitter-v1.png` | 1536 × 1024 | 4 × 2: idle · charge amber · charge full · fire · recoil · cooldown · port shut · arc projectile | P2 |
| `enemy-claw-v1.png` | 1536 × 1024 | 4 × 2: open idle · tracking · locked · descending · slam impact · gripping · retracting · rail carriage | P2 |
| `enemy-wasp-v1.png` | 1536 × 1024 | 4 × 2: flight A–D · spawn ember · dissolve A–C | P2 |
...
| File | Size | Cells | Pri |
|---|---|---|---|
| `furnace-background-v1.png` | 2172 × 724 | single plate | P1 |
| `furnace-platform-atlas-v1.png` | 1536 × 1024 | 3 × 2: slab wide · slab mid · ledge · slab worn · slab grated · ledge rail | P1 |
| `furnace-prop-atlas-v1.png` | 1536 × 1024 | 4 × 2: vent housing · ACT terminal · cooling shutter · coolant valve · laser emitter · power socket · emergency lift · pipe gantry | P1 |
| `casting-mold-v1.png` | 1254 × 1254 | mold platform | P2 |
| `coolant-mist-v1.png` | 1536 × 1024 | 4 × 2: burst 1–4 · fade 1–4 | P3 |
| `conveyor-belt-v1.png` | 1536 × 256 | tiling strip, 384 px repeat | P3 |
| `lava-channel-v1.png` | 1536 × 256 | tiling strip, 384 px repeat | P3 |
...
## Placeholder plates (shipped)

All 13 plates above now exist in `dist/assets/` as **greybox placeholders**: flat-colour silhouettes at the exact sizes and cell layouts specified here, transparent PNG-24, with no baked shadow or glow. They let Level 2 be built and played before any art is painted.

Regenerate them with `python design/build_level2_placeholders.py` (needs Pillow and NumPy). `design/level2-plate-contact-sheet.png` shows all 13 on one sheet.

Verified on generation: exact dimensions, RGBA, no subject touching a cell edge, and a 0-pixel seam difference on the three tiling plates, so the background and strips repeat cleanly.

They total **688 KB** for all 13, against roughly 15 MB had they been painted at Level 1's current sizes. That gap is the weight budget below, in miniature.

To replace one with real art, overwrite the file at the same size and cell layout. No code change is needed, because the game measures alpha bounds at load. Bump to `-v2` if the game has already shipped with the placeholder, so a cached browser cannot serve a stale mix.

## Art direction

Industrial semi-realistic, the same world as Level 1 one floor down and much hotter. Brighter molten metal, deeper shadow, and platform edges that read instantly.
...
This is a change to Level 1's shipped files, so it needs a decision before anyone acts on it.

## Open decisions

- Does the weight-budget pass happen before Level 2 art starts, or after?
- `bix-hang-v1.png` ships today but is never referenced. Wire it up, or drop it from `dist/`?
- Is the supervisor a head unit on a gantry only, or does it need a full body for the finale?
